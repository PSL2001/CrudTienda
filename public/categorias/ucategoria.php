...
$datosCat = (new Categorias)->read($_GET['id']);

$error = false; //Imaginemos que no hay ningun error en el documento
function hayError($nombre, $valor, $length)
{
    global $error;
    global $id; //Hay que pasarle ademas el id para comprobar si existe ese nombre en otras id's
    if (strlen($valor) < $length) { //Si el valor de la variable es menor a la longitud
        ...
        if ($nombre == "nombre") {
            if ((new Categorias)->setId($id)->existeCategoria($valor)) {
                $error = true;
            ...
            <form action="<?php echo $_SERVER['PHP_SELF']."?id=$id" ?>" method="post">
                <div class="m-auto bg-primary text-white rounded p-4 shadow-lg" style="width: 48rem;">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="nombre" name="nombre" placeholder="Introduce el nombre aqui" value="<?php echo $datosCat->nombre ?>" required>
                        <?php
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="descripcion" name="descripcion" rows="3"><?php echo $datosCat->descripcion ?></textarea>
                        <?php
                    ...
                    <div class="mt-3">
                        <button type="submit" class="btn btn-outline-light" name="btnUpdate"><i class="fas fa-pen-alt"></i> Editar</button>
